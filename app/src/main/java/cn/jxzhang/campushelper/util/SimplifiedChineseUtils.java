...
/**
 * Created on 2016/1/8 20:58
 * <p>Title:       SimplifiedChineseUtils</p>
 * <p>Description: [Description]</p>
 *
 * @author <a href=zhangjx_dev@163.com>j.x.zhang</a>
 * @version 1.0
 */
public class SimplifiedChineseUtils {

    private static LinkedHashMap<String, Integer> SPELL_MAP = null;

    static {
        if (SPELL_MAP == null) {
            SPELL_MAP = new LinkedHashMap<>(500);
        }
        initialize();
    }

    private SimplifiedChineseUtils() { /* cannot be instantiated */ }
     ...
     */
    public static String spellByAscii(int ascii) {
        ...
        while (it.hasNext()) {
            ...
            Object valObj = SPELL_MAP.get(spell);
            if (valObj != null) {
     ...
     * 转换成全拼后的字符串
     */
    public static String fullSpell(String cnStr) {
        ...
        for (char aChar : chars) {
            ...
            if (ascii == 0) {
                ...
                String spell = spellByAscii(ascii);
                if (spell == null) {
                    result.append(aChar);
                } else {
                    result.append(spell);
     ...
     * @return 返回转换后的拼
     */
    public static String firstSpell(String cnStr) {
        if (null == cnStr || "".equals(cnStr.trim())) {
            return cnStr;
        }
